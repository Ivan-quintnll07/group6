    ...
    @foreach ($offers as $category => $categoryOffers)
      ...
      <div class="grid gap-10 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
        @foreach ($categoryOffers as $index => $offer)
          <div class="group relative bg-white rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition animate-fade-up animate-delay-{{ $index * 100 }}">
...
=======
  <!-- Hero banner -->
  <section class="w-full h-64 bg-gradient-to-r from-cyan-700 to-blue-600 flex items-center justify-center shadow-lg mb-16">
    <h1 class="text-5xl font-extrabold text-white tracking-widest drop-shadow-md">PROMOCIONES</h1>
  </section>
    ...
    @foreach ($offers as $category => $categoryOffers)
      ...
      <div class="grid gap-10 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
        @foreach ($categoryOffers as $offer)
          <div class="group relative bg-white rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition">
            <!-- Image with overlay -->
            <div class="relative h-48 overflow-hidden">
              <img src="{{ asset('img/' . $offer->image) }}" alt="{{ $offer->title }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-300">
              <span class="absolute top-2 right-2 bg-white/80 backdrop-blur-sm px-2 py-0.5 rounded-full text-xs font-semibold text-gray-800 shadow">{{ $offer->category }}</span>
            </div>

            <!-- Card content -->
            <div class="p-5 space-y-2 flex flex-col h-56">
              <h3 class="text-xl font-bold line-clamp-2 text-gray-900">{{ $offer->title }}</h3>
>>>>>>> Stashed changes
=======
  <!-- Hero banner -->
  <section class="w-full h-64 bg-gradient-to-r from-cyan-700 to-blue-600 flex items-center justify-center shadow-lg mb-16">
...
<<<<<<< Updated upstream
                ...
                <form action="{{ route('offers.toggleFavorite', $offer) }}" method="POST">
                  ...
                  <button type="submit" aria-label="Marcar favorito" class="rounded-full p-2 hover:bg-gray-100 transition">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-5xl transition-colors duration-300 ease-in-out @auth @if(auth()->user()->favorites->contains($offer->id)) text-red-500 @else text-gray-400 @endif @else text-gray-400 @endauth" fill="currentColor" viewBox="0 0 24 24" stroke="none">
=======
=======
>>>>>>> Stashed changes
                <!-- Favorite button -->
                <form action="{{ route('offers.toggleFavorite', $offer) }}" method="POST">
                  @csrf
                  <button type="submit" aria-label="Marcar favorito" class="rounded-full p-2 hover:bg-gray-100 transition">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 @auth @if(auth()->user()->favorites->contains($offer->id)) text-red-500 @else text-gray-400 @endif @else text-gray-400 @endauth" fill="currentColor" viewBox="0 0 24 24" stroke="none">
<<<<<<< Updated upstream
>>>>>>> Stashed changes
=======
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
                ...
                <a href="{{ route('offers.show', $offer) }}" class="inline-flex items-center gap-2 bg-cyan-600 hover:bg-cyan-700 text-white text-sm font-semibold px-4 py-2 rounded-full shadow transition transform hover:scale-105">
                  Ver más
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 animate-slide-right" fill="none" viewBox="0 0 24 24" stroke="currentColor">
=======
=======
>>>>>>> Stashed changes
                <!-- View more button -->
                <a href="{{ route('offers.show', $offer) }}" class="inline-flex items-center gap-2 bg-cyan-600 hover:bg-cyan-700 text-white text-sm font-semibold px-4 py-2 rounded-full shadow transition">
                  Ver más
