<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Document</title>
    <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">
</head>

<body>

<header class="header bg-gray-900 text-white">

    <div class="menu container mx-auto flex justify-between items-center py-4">
        <a href="#" class="logo text-xl font-bold">Logo</a>
        <input type="checkbox" id="menu" class="hidden" />
        <label for="menu" class="cursor-pointer md:hidden">
            <img src="data:image/png;base64," alt="Menu">
        </label>
        <nav class="navbar hidden md:block">
            <ul class="flex space-x-4">
                <li><a href="#" class="hover:text-gray-300">Sobre nosotros</a></li>
                <li><a href="#" class="hover:text-gray-300">Registrarse</a></li>
                <li><a href="#" class="hover:text-gray-300">Iniciar sesion</a></li>
            </ul>
        </nav>
    </div>
</header>

<section class="hero bg-blue-900 text-white py-20">
    <div class="container mx-auto">
        <h1 class="text-4xl font-bold">WELCOME</h1>
        <p class="mt-4">  ¿Quiere encontrar fácilmente descuentos para sus tarjetas de crédito en una sola página web? ¡Está en el lugar indicado!.</p>
    </div>
</section>

<

<footer class="footer bg-gray-900 text-white py-10">
    <div class="container mx-auto text-center">
        <p>&copy; ola</p>
    </div>
</footer>

</body>
</html>
